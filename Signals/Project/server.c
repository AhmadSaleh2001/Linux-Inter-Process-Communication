...
int connected_processes[MAX_CLIENT_SUPPORTED];
mac_t *mac_table_entries;
ipAddress_t *ip_table_entries;
...
static void syncWithClients(OPCODE op, mac_t newToAdd)
{

    int i = 0;
    sync_mac_t *addMsg = malloc(sizeof(sync_mac_t));
    addMsg->mac = newToAdd;
    addMsg->op = op;
    for (; i < MAX_CLIENT_SUPPORTED; i++)
    {
        if (monitored_fd_set[i] == 0 || monitored_fd_set[i] == -1)
            continue;
        write(monitored_fd_set[i], addMsg, BUFFER_SIZE);
    }
}

void syncAllDataWithClient(mac_t **arr, int sz, int clientfd)
{
    char allDataToSend[BUFFER_SIZE];
    (*allDataToSend) = SYNC;
    for (int i = 0; i < sz; i++)
    {
        if ((*arr)[i].macAddress[0] == 0)
            continue;
        int ret = write(monitored_fd_set[clientfd], *arr, BUFFER_SIZE);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    ip_table_entries = malloc(sizeof(ipAddress_t) * NUMBER_OF_ENTRIES);
    memset(ip_table_entries, 0, sizeof(ip_table_entries));

    memset(connected_processes, -1, sizeof(connected_processes));
    ...
    for (;;)
    {
        ...
        if (FD_ISSET(0, &readfds))
        {
            ...
            if (choice == 1)
            {
                ...
                strcpy(ipAddress.macAddress, newToAdd.macAddress);
                enter_ipAddress(&ipAddress);

                addNewMac(&mac_table_entries, NUMBER_OF_ENTRIES, newToAdd);
                addNewIpEntry(&ip_table_entries, NUMBER_OF_ENTRIES, ipAddress);

                saveItOnFile(&ip_table_entries, NUMBER_OF_ENTRIES, "test.txt");
            ...
            else if (choice == 2)
            {
                mac_t newToAdd;
                enter_mac(&newToAdd);
                updateMsg(&mac_table_entries, NUMBER_OF_ENTRIES, newToAdd);
            ...
            else if (choice == 3)
            {
                mac_t newToAdd;
                enter_mac(&newToAdd);
                removeMsg(&mac_table_entries, NUMBER_OF_ENTRIES, newToAdd);
                saveItOnFile(&mac_table_entries, NUMBER_OF_ENTRIES, "test.txt");
            ...
            {
                // lets kill everything, also our connected processes
                for (int i = 1; i < MAX_CLIENT_SUPPORTED; i++)
                {
                    if (monitored_fd_set[i] != -1 && connected_processes[i] != -1)
                    {
                        comm_socket_fd = monitored_fd_set[i];
                        printf("killing %d\n", connected_processes[i]);
                        remove_from_monitored_fd_set(comm_socket_fd);
                        kill(connected_processes[i], SIGUSR1);
                    }
                }
                goto closeMaster;
            }
        ...
        else if (FD_ISSET(connection_socket, &readfds))
        {
            ...
            syncAllDataWithClient(&mac_table_entries, NUMBER_OF_ENTRIES, clientfd);
        }
        else
        {
            ...
            for (; i < MAX_CLIENT_SUPPORTED; i++)
            {
                ...
                if (FD_ISSET(monitored_fd_set[i], &readfds))
                {
                    ...
                    printf("connect process id: %d\n", data);
                    connected_processes[i] = data;
                }
            }
        }
