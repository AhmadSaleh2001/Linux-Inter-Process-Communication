...
void setUpUnixDomainSockets(int *connection_socket, struct sockaddr_un *name)
{
  ...
  ret = bind((*connection_socket), name,
...
void setUpMessageQueue(struct mq_attr *attr, int *msgq_fd, fd_set *readfds)
{
...
int main(int argc, char *argv[])
{
  ...
  int ret;
  int connection_socket_unix_domain_socket;
  int data_socket;
  int result;
  int data;
  ...
  setUpUnixDomainSockets(&connection_socket_unix_domain_socket, &name);
  ...
  setUpMessageQueue(&attr, &msgq_fd, &readfds);
  ...
  for (;;)
  {
    ...
    else if (FD_ISSET(connection_socket_unix_domain_socket, &readfds))
    {
      ...
      data_socket = accept(connection_socket_unix_domain_socket, NULL, NULL);
  ...
  close(connection_socket_unix_domain_socket);
  remove_from_monitored_fd_set(connection_socket_unix_domain_socket);
