...
static void syncWithClientsMac_IP(OPCODE op, ipAddress_t *toSend)
{
    ...
    for (; i < MAX_CLIENT_SUPPORTED; i++)
    {
        if (monitored_fd_set[i] == 0)
            continue;
        write(monitored_fd_set[i], toSend, BUFFER_SIZE);
    }
}
    ...
    for (;;)
    {
        ...
        if (FD_ISSET(0, &readfds))
        {
            ...
            else if (choice == 5)
            {
                ...
                printf("\n");
                syncWithClientsMac_IP(PRINT_WITH_IP, ipAddress);
            }
        }
